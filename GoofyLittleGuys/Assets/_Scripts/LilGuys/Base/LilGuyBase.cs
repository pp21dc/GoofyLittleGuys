    ...
    public virtual void PlayDeathAnim(bool isWild = false)
    {
		...
		if (!isWild)
        {
			...
			if (h.LastHit != null)
            {
                ...
                {
                    h.LastHit.LilGuyTeam[i].AddXP((i ==0) ? Mathf.FloorToInt((Mathf.Pow((Level + 8), 2)) / 3) : Mathf.FloorToInt((Mathf.Pow((Level + 6), 2)) / 4));
				}
            }
        ...
        else
        {
            ...
            for (int i = 0; i < h.LastHit.LilGuyTeam.Count; i++)
			{
				h.LastHit.LilGuyTeam[i].AddXP((i == 0) ? Mathf.FloorToInt((Mathf.Pow((Level + 6), 2) / 4)) : Mathf.FloorToInt((Mathf.Pow((Level + 4), 2) / 5)));
			}
