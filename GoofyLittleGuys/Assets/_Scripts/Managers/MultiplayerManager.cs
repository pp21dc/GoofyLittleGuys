...
namespace Managers
{
	public class MultiplayerManager : SingletonBase<MultiplayerManager>
	{
		...
		public void OnPlayerJoinedLeft(PlayerInput input)
		{
			if (!canJoinLeave) return; 
			if (PlayerInputManager.instance.playerCount == 3)
			{
				PlayerInput.all[2].GetComponentInChildren<Camera>().rect = new Rect(0.25f, 0, 0.5f, 0.5f);
			}
		}
		...
		/// </summary>
		private void AdjustCameraRects()
		{
			...
			if (playerCount == 3)
			{
				...
				PlayerInput.all[2].GetComponentInChildren<Camera>().rect = new Rect(0.25f, 0, 0.5f, 0.5f);
				PlayerInput.all[1].GetComponentInChildren<Camera>().rect = new Rect(0.5f, 0, 0.5f, 0.5f);
				PlayerInput.all[0].GetComponentInChildren<Camera>().rect = new Rect(0f, 0, 0.5f, 0.5f);
			}
			else if (playerCount == 2)
			{
				...
				{
					PlayerInput.all[i].GetComponentInChildren<Camera>().rect = new Rect(i * 0.5f, 0, 0.5f, 1f);
				}
			}
			else if (playerCount == 1)
			{
				// Full screen view
				PlayerInput.all[0].GetComponentInChildren<Camera>().rect = new Rect(0, 0, 1f, 1f);
			}
		}
